...
import 'package:listwhatever/helpers/date_helper.dart';
...
class FiltersHelper {
  ...
  static List<ListItem> filterListItems({
    ...
    required LatLng? distanceFilterCenter,
  }) {
  ...
  static bool matchesFilter({
    ...
    final matches = matchesDate && matchesCategories && matchesDistance && matchesItemName;
    //
  ...
  static bool matchesDatesFilter({
    ...
    final response = item.datetime!.compareTo(filters.startDate ?? DateHelper.minDateTime) >= 0 &&
        item.datetime!.compareTo(filters.endDate ?? DateHelper.maxDateTime) <= 0;
    if (!response) {
      //
    }
    return response;
  ...
  static bool matchesRegularCategoriesFilter(ListItem item, Filters filters) {
    ...
    */
    if (filters.regularCategoryFilters == null || filters.regularCategoryFilters!.values.expand((e) => e).isEmpty) {
      //
      return true;
    }
    if (filters.regularCategoryFilters!.values.expand((e) => e).isNotEmpty && item.categories.isEmpty) {
      //
      return false;
    }
  ...
  static bool matchesDateCategoriesFilter(ListItem item, Filters filters) {
    final categoriesWithDates = filters.dateCategoryFilters?.entries.map((e) => e.key).toList();
    ...
    for (final entry in dateCategories) {
      ...
      final boundryDates =
          (DateTime.fromMillisecondsSinceEpoch(boundries!.$1), DateTime.fromMillisecondsSinceEpoch(boundries.$2));
      for (final date in entry.$2) {
        if (date.compareTo(boundryDates.$1) >= 0 && date.compareTo(boundryDates.$2) <= 0) {
          return true;
        }
      }
      return false;
  ...
  static bool matchesTimeOfDayCategoriesFilter(ListItem item, Filters filters) {
    final categoriesWithTimeOfDay = filters.timeOfDayCategoryFilters?.entries.map((e) => e.key).toList();
    ...
    for (final entry in timeOfDayCategories) {
      ...
      for (final timeOfDay in entry.$2) {
        if (timeOfDay >= boundryTimeOfDays.$1 && timeOfDay <= boundryTimeOfDays.$2) {
          return true;
        }
      }
      return false;
  ...
  static bool matchesNumericCategoriesFilter(ListItem item, Filters filters) {
    final categoriesWithNumeric = filters.numericCategoryFilters?.entries.map((e) => e.key).toList();
  ...
  static bool matchesDistanceFilter({
    ...
    final gcd = GreatCircleDistance.fromDegrees(
      latitude1: item.latLong!.latitude,
      longitude1: item.latLong!.longitude,
      latitude2: distanceFilterCenter.latitude,
      longitude2: distanceFilterCenter.longitude,
    );
    final response = gcd.haversineDistance() >= filters.distance!.$1 && gcd.haversineDistance() <= filters.distance!.$2;
    return response;
  }
  ...
  static bool matchesItemNameFilter(
    ...
    final result = item.name.toLowerCase().contains(filters.itemName!.toLowerCase());
    return result;
  }
}
